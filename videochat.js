...
peer.on('connection',function(conn){
        connectedConn = conn;
        conn.on('open',function(){      //コネクション利用可能に
        conn.on('data',function(data){  //データ受信したと
...
$(function() {
    ...
    $('#call-start').click(function(){
        ...
        connectedCall=call;
        connectedConn=conn;
            //これでいいのおおおおおおおおおおおおおおおおお
        // 相手のストリームが渡された場合、このstreamイベントが呼ばれる
        // - 渡されるstreamオブジェクトは相手の映像についてのストリームオブジェクト
        $("#peer-id").text(call.peer);
    //    $("#peer-id2").text(conn.peer);
        call.on('stream', function(stream){
            // 相手のIDを表示する
            // 映像ストリームオブジェクトをURLに変換する
            // - video要素に表示できる形にするため変換している
        ...
        connectedConn.on("open",function(){
        connectedConn.on("data",function(data){
                $("#data-received").text(data);
        });
        });
    });
...
$("#loadFile").onchange = function(evt){
        ...
        reader.onload = function(evt) {
                var abf = evt.target.result;
                $("#info").value = abf;
                connectedConn.send(abf);
        }
